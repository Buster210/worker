...
describe('perf cache seams', () => {
  it('caches repo assertions per directory', () => {
    ...
    const initResult = childProcess.spawnSync('git', ['init', '-q'], { cwd: dir, stdio: 'ignore' });
    ...
    try {
      ...
      const gitChecks = spy.mock.calls.filter(([cmd, args]) =>
        cmd === 'git'
        && Array.isArray(args)
        && args[0] === 'rev-parse'
  ...
  it('skips ps when liveness is checked without a start time', () => {
    ...
    const spy = spyOn(childProcess, 'spawnSync').mockImplementation(mockSpawnSync);
  ...
  it('uses the login-env disk cache and respawns on stale signature', () => {
    ...
    try {
      ...
      writeFileSync(cachePath, JSON.stringify({ shell, sig: loginEnvSig(shell), env: { FOO: 'cached' } }));
      ...
      writeFileSync(cachePath, JSON.stringify({ shell, sig: 'STALE', env: { FOO: 'cached' } }));
      __resetLoginEnvCache();
