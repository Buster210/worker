...
describe("resolveStatus", () => {
  ...
  it('omp json ending assistant text "FAILED:reason" -> failed:reason', () => {
    ...
    const events = [
      JSON.stringify({
        message: {
          role: "assistant",
          content: [{ type: "text", text: "attempting task" }],
        },
      }),
      JSON.stringify({
        message: {
          role: "assistant",
          content: [{ type: "text", text: "FAILED: rate limit exceeded" }],
        },
      }),
    ].join("\n");
    writeFileSync(tmpLogPath, events);
    testLogs.push(tmpLogPath);
    expect(resolveStatus("omp", 0, tmpLogPath, false)).toBe(
  ...
  it("omp json with provider errorStatus (e.g. 402 credits) -> failed:<message>", () => {
    ...
    const events = [
      ...
      JSON.stringify({
        ...
        stopReason: "error",
        errorStatus: 402,
        errorMessage: "402 Add credits to continue",
      }),
    ...
    testLogs.push(tmpLogPath);
    expect(resolveStatus("omp", 0, tmpLogPath, false)).toBe(
      "failed:402 Add credits to continue",
    );
  });
  ...
  it("codex json with agent_message containing FAILED:<reason> -> failed:<reason>", () => {
    ...
    const events = [
      ...
      JSON.stringify({
        type: "item.completed",
        item: { id: "i1", type: "agent_message", text: "FAILED: bad prompt" },
      }),
      JSON.stringify({ type: "turn.completed", usage: {} }),
    ].join("\n");
    writeFileSync(tmpLogPath, events);
    testLogs.push(tmpLogPath);
    expect(resolveStatus("codex", 0, tmpLogPath, false)).toBe(
      "failed:bad prompt",
    );
  });
  ...
  it("pool json with thought containing FAILED -> failed", () => {
    ...
    writeFileSync(tmpLogPath, events);
    testLogs.push(tmpLogPath);
    expect(resolveStatus("pool", 0, tmpLogPath, false)).toBe(
      "failed:out of context",
    );
  });
