...
function seedLadder(lastResult: string): {
  handle: string;
  lockPath: string;
  sid: string;
} {
...
describe("statusLine", () => {
  it("collapses done to pointer wording", () => {
    expect(statusLine("done")).toBe(
      "completed — worker committed changes to branch current branch. Review the diff below and merge; nothing else to run.",
    );
    expect(statusLine("done", "worker/h1", "abc123")).toBe(
      "completed — worker committed changes to branch worker/h1 (base abc123). Review the diff below and merge; nothing else to run.",
    );
  });
...
describe("wantsDiff", () => {
  it("omits the diff only for stopped/killed", () => {
    ...
    for (const s of ["done", "failed:max-turns", "timeout", "exhausted"])
      expect(wantsDiff(s)).toBe(true);
  });
  it("done → pointer wording + worktree + branch + full diff", () => {
    const { handle, lockPath } = seedRun("done");
    const ptr =
      "completed — worker committed changes to branch current branch. Review the diff below and merge; nothing else to run.";
    expect(renderReport(handle, lockPath, () => "DIFFBODY")).toBe(
  ...
  it("exhausted ladder → headline + per-rung breakdown + diff", () => {
    ...
    const expected =
      "LADDER EXHAUSTED — no backend completed the task. Breakdown:\n" +
      "  rung 1: omp → failed\n" +
  ...
  it("done with empty diff emits a warning", () => {
    ...
    const ptr =
      "completed — worker committed changes to branch current branch. Review the diff below and merge; nothing else to run.";
    expect(renderReport(handle, lockPath, () => "(no tracked changes)")).toBe(
  ...
  it("unknown handle on a diff status → sentinel, never a wrong-repo diff (diff fn not invoked)", () => {
    let called = false;
    const r = renderReport("ghost", "/any/ghost/.lock", () => {
      called = true;
      return "WRONGDIFF";
    });
...
describe("waitForUnlock — anti-hang", () => {
  ...
  it("returns immediately when the lock is already gone", async () => {
    await waitForUnlock(
      join(STATE_DIR, `${uniq("lk")}.missing.lock`),
      process.pid,
    );
  ...
  it("unknown owner (serverPid 0) does NOT bail — only the lock removal resolves it", async () => {
    const lock = join(STATE_DIR, `${uniq("lk")}.lock`);
    writeFileSync(lock, "");
    let resolved = false;
    const p = waitForUnlock(lock, 0).then(() => {
      resolved = true;
    });
    await Bun.sleep(80);
...
describe("isNearTimeout — band predicate", () => {
  const near = 30_000;
  const run = (deadline_at?: number, status = "running") => ({
    status,
    deadline_at,
  });
