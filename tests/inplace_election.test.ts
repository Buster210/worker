...
import { insertJob, updateJob, isInPlaceOwner, workersDir, __resetStateForTest } from '../src/state.ts';
...
function makeJob(handle: string, created_at: number, opts?: { server_pid?: number; server_started?: string }) {
  insertJob({
    handle,
    backend: 'cmd',
    sid: 'test-sid',
...
describe('isInPlaceOwner', () => {
  ...
  it('dead-server job does not block election', () => {
    const ts = Date.now();
    makeJob('dead-job', ts, { server_pid: 999999, server_started: '2000-01-01T00:00:00.000Z' });
