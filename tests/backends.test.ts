...
describe("buildRunArgv", () => {
  ...
  it("throws for unknown backend and lists valid backends", () => {
    expect(() =>
      buildRunArgv("bogus" as Backend, "spec", "/repo", "sid"),
...
describe("buildResumeArgv", () => {
  ...
  it("builds omp resume argv with the same session dir plus --continue", () => {
    ...
    expect(argv).toEqual([
      ...
      "spec",
      "--session-dir",
      handleDirUncached("token123", "/repo"),
      "--continue",
      "--approval-mode=yolo",
      "--mode=json",
    ]);
  ...
  it("throws for unknown backend and lists valid backends", () => {
    expect(() =>
      buildResumeArgv("bogus" as Backend, "spec", "/repo", "token"),
    ).toThrow(`Unknown backend: bogus. Valid: ${ALL_BACKENDS.join(", ")}`);
  });
});
...
describe("QUIET_BACKENDS", () => {
  it("includes codex", () => {
    expect(QUIET_BACKENDS.has("codex")).toBe(true);
  });
  it("excludes claude", () => {
    expect(QUIET_BACKENDS.has("claude")).toBe(false);
  });
  it("excludes cmd", () => {
    expect(QUIET_BACKENDS.has("cmd")).toBe(false);
  });
  it("excludes pool", () => {
    expect(QUIET_BACKENDS.has("pool")).toBe(false);
  });
});
...
describe("stall thresholds", () => {
  ...
  it("stallTimeoutMs defaults to 60_000", () => {
    expect(stallTimeoutMs()).toBe(60_000);
  });
  it("quietStallMs defaults to 240_000", () => {
    expect(quietStallMs()).toBe(240_000);
  });
