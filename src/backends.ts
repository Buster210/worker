...
export const ALL_BACKENDS: readonly Backend[] = [
  "codex",
  "cmd",
  "pool",
  "omp",
...
export function buildRunArgv(
  ...
  switch (backend) {
    ...
    default:
      throw new Error(
        `Unknown backend: ${backend}. Valid: ${ALL_BACKENDS.join(", ")}`,
      );
  }
}
...
export function emitsJsonLog(backend: string): boolean {
  return backend === "omp" || backend === "codex" || backend === "pool";
}
