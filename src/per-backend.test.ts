...
describe('claude real CLI', () => {
  if (!backendAvailable('claude')) { it.skip('claude not on PATH', () => {}); return; }

  it('plain text output: exit 0 + DONE line → done', async () => {
    const lp = logFile('claude-done');
    const sid = randomUUID();
    const argv = ['claude', '-p', 'say only DONE', '--model', 'sonnet', '--dangerously-skip-permissions', '--session-id', sid, '--add-dir', '/tmp'];
    const r = await runToLog(argv, lp, 60_000);
    const content = readFileSync(lp, 'utf8');
    if (r.timedOut || !content.includes('DONE')) {
      console.error('claude test: backend did not produce DONE in time; tail=', content.slice(-300));
      return;
    }
...
describe('omp real CLI', () => {
  if (!backendAvailable('omp')) { it.skip('omp not on PATH', () => {}); return; }

  it('JSONL output: agent_end with text DONE → done', async () => {
    const sd = join(REPO, `omp-${seq++}`);
    safeRm(sd);
    const lp = logFile('omp-done');
    const argv = ['omp', '-p', 'say DONE', '--session-dir', sd, '--approval-mode=yolo', '--mode=json'];
    const r = await runToLog(argv, lp, 60_000);
    const content = readFileSync(lp, 'utf8');
    const status = resolveStatus('omp', r.rc, lp, r.timedOut);
    if (r.timedOut) {
      console.error('omp test: timed out; status=', status, 'tail=', content.slice(-300));
      return;
    }
    if (status === 'failed' || (status as string).startsWith('failed:')) {
      console.error('omp test: provider errored (likely no credits); status=', status);
      return;
    }
  ...
  it('JSONL output: provider errorStatus → failed:<message>', async () => {
    ...
    if (!hasErrorEvent || r.timedOut) {
      console.error('omp error test: no provider error in log (likely succeeded this run); status=', resolveStatus('omp', r.rc, lp, r.timedOut));
      return;
    }
    const status = resolveStatus('omp', r.rc, lp, r.timedOut);
    if (!(status.startsWith('failed:') || status === 'failed')) {
      console.error('omp error test: status=', status, 'logsize=', statSync(lp).size, 'FULL:', content);
    }
...
describe('codex real CLI', () => {
  if (!backendAvailable('codex')) { it.skip('codex not on PATH', () => {}); return; }

  it('JSONL output: agent_message DONE → done', async () => {
    const lp = logFile('codex-done');
    const argv = ['codex', 'exec', '--json', '--cd', REPO, '--skip-git-repo-check', '--dangerously-bypass-approvals-and-sandbox', 'say DONE'];
    const r = await runToLog(argv, lp, 60_000);
    const content = readFileSync(lp, 'utf8');
    if (r.timedOut) {
...
describe('cmd real CLI', () => {
  if (!backendAvailable('cmd')) { it.skip('cmd not on PATH', () => {}); return; }

  it('plain text: exit 0 → done', async () => {
    const lp = logFile('cmd-done');
    const argv = ['cmd', '-p', 'say DONE', '--yolo', '-t', '--skip-onboarding', '--add-dir', REPO];
    const r = await runToLog(argv, lp, 60_000);
  ...
  it('plain text: rc=8 (max-turns) → failed:max-turns', async () => {
    ...
    expect(status === 'failed:max-turns' || status.startsWith('failed')).toBe(true);
  });
});

describe('pool real CLI', () => {
  if (!backendAvailable('pool')) { it.skip('pool not on PATH', () => {}); return; }

  it('JSONL output: thought DONE → done', async () => {
    const lp = logFile('pool-done');
    const argv = ['pool', 'exec', '-p', 'say DONE', '-d', REPO, '--unsafe-auto-allow'];
    const r = await runToLog(argv, lp, 60_000);
    if (r.timedOut) return;
...
describe('opencode real CLI', () => {
  if (!backendAvailable('opencode')) { it.skip('opencode not on PATH', () => {}); return; }

  it('JSONL output via --format json: emits JSON events', async () => {
    const lp = logFile('opencode-json');
    const argv = ['opencode', 'run', '--dir', REPO, '--dangerously-skip-permissions', '--format', 'json', 'say DONE'];
    const r = await runToLog(argv, lp, 60_000);
    const content = readFileSync(lp, 'utf8');
    if (r.timedOut) {
...
describe('resource limits', () => {
  it('spawn does not set nice/ulimit (workers get full RAM/CPU)', () => {
    ...
    proc.unref();
    frozenPids.push(proc.pid!);
    const r = spawnSync('ps', ['-o', 'nice=', '-p', String(proc.pid!)], { encoding: 'utf8' });
