...
function seedRunning(fields: {
  ...
  insertJob({
    ...
    server_pid: job.server_pid,
    server_started: job.server_started,
    server_sid: job.server_sid,
  });
  return handle;
}

function ensureActiveDir(): void {
  mkdirSync(ACTIVE_DIR, { recursive: true });
}
function writeClientPid(pid: number): void {
  ensureActiveDir();
  writeFileSync(join(ACTIVE_DIR, String(pid)), "");
}
function removeClientPid(pid: number): void {
  try {
    rmSync(join(ACTIVE_DIR, String(pid)));
  } catch {}
}
function cleanupActiveDir(): void {
  try {
...
describe("sweepStaleWorkerDirs", () => {
  it("cleans terminal job dirs but preserves killed:no-client dirs", () => {
    const doneHandle = seedRunning({
      worker_pid: deadPid(),
      status: "done",
      finished: new Date(Date.now() - 8 * 86_400_000).toISOString(),
    });
    const killedHandle = seedRunning({
      worker_pid: deadPid(),
      status: "killed:no-client",
      finished: new Date(Date.now() - 8 * 86_400_000).toISOString(),
    });
    const runningHandle = seedRunning({
      worker_pid: spawnSleep(),
      status: "running",
    });
    ...
    expect(existsSync(join(workersDir(), "test-repo", killedHandle))).toBe(
      true,
    );
    expect(existsSync(join(workersDir(), "test-repo", runningHandle))).toBe(
      true,
    );
  });
