...
export function tailCapped(
  logPath: string,
  cap: number = STATUS_TAIL_BYTES,
): string {
  try {
...
export function readSentinel(
  logPath: string,
  json: boolean,
): { status: SentinelResult; lastText: string } {
  let st: { size: number; mtimeMs: number };
  try {
    st = statSync(logPath);
  } catch {
    return { status: null, lastText: "" };
  }
  ...
  const suffixCandidates = json
    ? suffixLines.flatMap(extractAssistantTexts)
    : suffixLines;
  ...
  if (!status && st.size > SENTINEL_TAIL_BYTES) {
    ...
    const fullCandidates = json
      ? fullLines.flatMap(extractAssistantTexts)
      : fullLines;
...
export function __resetSentinelCache(): void {
  ...
  for (let i = lines.length - 1; i >= 0; i--) {
    ...
    const message =
      msgObj && typeof msgObj === "object"
        ? (msgObj as Record<string, unknown>)
        : null;
    const stopReason =
      (obj as { stopReason?: unknown }).stopReason ?? message?.stopReason;
    if (
      stopReason === "error" ||
      stopReason === "refusal" ||
      ...
      const msg =
        (obj as { errorMessage?: unknown }).errorMessage ??
    ...
    const errorStatus =
      (obj as { errorStatus?: unknown }).errorStatus ?? message?.errorStatus;
