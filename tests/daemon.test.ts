...
function waitForOutput(
  ...
  const timer = setTimeout(
    () => reject(new Error("Timeout waiting for daemon output")),
    timeoutMs,
  );
  let buffer = "";
...
async function stopDaemon(proc: ChildProcess): Promise<void> {
  const { promise: exitWait, resolve: exitResolve } =
...
async function mcpInitialize(
  ...
  if (res.status !== 200)
    throw new Error(`initialize failed: ${res.status} ${res.body}`);
...
describe("live daemon", () => {
  ...
  it("stale lockfile with dead PID → reclaims on restart", async () => {
    ...
    writeFileSync(
      TEST_LOCK_PATH,
      JSON.stringify({ pid: 99999, port, started_at: "2020-01-01T00:00:00Z" }),
    );
  ...
  it("port occupied by non-daemon → daemon detects and retries", async () => {
    ...
    const { promise: blockerReady, resolve: ready } =
      Promise.withResolvers<void>();
    ...
    const { promise: blockerDone, resolve: done } =
      Promise.withResolvers<void>();
    blocker.close(() => done());
    await blockerDone;
  ...
  it("two sessions, disconnect one → other stays; last out keeps daemon alive", async () => {
    ...
    try {
      await httpDelete(port, "/mcp", { "mcp-session-id": s1.sessionId });
    } catch {}
    await new Promise((r) => setTimeout(r, 500));
    expect(JSON.parse((await httpGet(port, "/health")).body).sessions).toBe(1);

    try {
      await httpDelete(port, "/mcp", { "mcp-session-id": s2.sessionId });
    } catch {}
    await new Promise((r) => setTimeout(r, 500));
    expect(existsSync(TEST_LOCK_PATH)).toBe(true);
    expect(JSON.parse((await httpGet(port, "/health")).body).sessions).toBe(0);
