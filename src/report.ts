...
export function statusLine(
  ...
  if (status === "exhausted" && ladderRuns?.length) {
    const breakdown = ladderRuns
      .map((r) => `  rung ${r.turn}: ${r.worker} → ${r.result}`)
      .join("\n");
...
function gitDiff(repo: string, baseSha?: string): string {
  ...
  if (r.error || typeof r.stdout !== "string") {
    console.error(
      `report: git diff failed in ${repo || "<empty repo path>"}: ${r.error?.message ?? "no stdout"}`,
    );
...
export function isNearTimeout(
  job: { status: string; deadline_at?: number } | null | undefined,
  now: number,
): boolean {
...
export async function waitForUnlock(
  ...
  serverPid = 0,
  handle?: string,
): Promise<"unlocked" | "near_timeout"> {
  if (!existsSync(lockPath)) return "unlocked";
  const { promise, resolve } = Promise.withResolvers<
    "unlocked" | "near_timeout"
  >();
  ...
  const poll = () => {
    ...
    if (handle && isNearTimeout(getJobFresh(handle), Date.now())) {
      settle("near_timeout");
      return;
    }
...
function gitDiffStat(repo: string): string {
  const r = spawnSync("git", ["-C", repo, "diff", "--stat"], {
    encoding: "utf8",
    timeout: 5000,
  });
...
export function nearTimeoutLine(
  handle: string,
  job: { log_path: string; worktree_path?: string; repo: string },
): string {
  const now = Date.now();
  const lastMtime = logMtimeMs(job.log_path);
  const secsSinceActivity =
    lastMtime > 0 ? Math.round((now - lastMtime) / 1000) : -1;
  ...
  const activityPart =
    secsSinceActivity >= 0
      ? `last-activity ${secsSinceActivity}s ago`
      : "no activity";
...
if (import.meta.main) {
  ...
  if (!wait && isNearTimeout(job, Date.now())) {
    ...
    const why = await waitForUnlock(
      lock,
      job.server_pid,
      wait ? undefined : handle,
    );
    if (why === "near_timeout") {
