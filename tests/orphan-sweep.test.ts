...
const THIS_SID =
  process.env.CLAUDE_CODE_SESSION_ID ?? `test-sid-${process.pid}`;
...
describe("sweepStaleJobs — orphan branch", () => {
  ...
  it("leaves an unknown-owner job (server_pid 0) with a live worker untouched", () => {
    const workerPid = spawnSleep();
  ...
  it("cleans an unknown-owner job (server_pid 0) whose worker is dead", () => {
    const handle = seedRunning({
      worker_pid: deadPid(),
      server_pid: 0,
    });
...
describe("sweepChainLocks", () => {
  ...
  beforeEach(() => {
    ...
    try {
      for (const f of readdirSync(ladder)) {
        if (f.endsWith(".chain.lock")) {
          try {
            unlinkSync(join(ladder, f));
          } catch {}
        }
      }
    } catch {}
  ...
  it("leaves an empty chain lock with a fresh mtime (within TTL)", () => {
    const sid = `test-empty-fresh-${process.pid}-${seq++}`;
...
describe("spawnReaper single-instance guard", () => {
  ...
  it("does not spawn a second reaper when a live pidfile already exists", () => {
    ...
    expect(readFileSync(reaperPidPath(), "utf8").trim()).toBe(
      String(process.pid),
    );
  });
});
...
describe("worktree reap — sweepStaleWorkerDirs and normal finalize", () => {
  ...
  it("ownsWorktree: true for the creator, false for a ladder reuse-sibling sharing the path", () => {
    ...
    insertJob({
      ...
      sid: "test",
      repo,
      log_path: stateLogPath(owner, repo),
    });
    ...
    insertJob({
      ...
      sid: "test",
      repo,
      log_path: stateLogPath(sibling, repo),
      worktree_path: wt,
    });
...
describe("getAllRunningJobsFresh — stale cache regression", () => {
  it("returns jobs created after cache bootstrap while cached getAllRunningJobs does not", () => {
    ...
    expect(getAllRunningJobsFresh().some((j) => j.handle === handle)).toBe(
      true,
    );
