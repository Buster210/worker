...
describe("isProcessAlive — cached per-pid etime (no repeated ps spawns)", () => {
  it("reuses the first verified start on subsequent calls for the same pid+started", () => {
    ...
    const psSpy = spyOn(childProcess, "spawnSync").mockImplementation(((
      cmd: string,
      _args: string[],
    ) => {
      if (cmd === "ps")
        return { status: 0, stdout: "00:01", stderr: "" } as ReturnType<
          typeof spawnSync
        >;
      if (cmd === "pgrep")
    ...
    try {
      ...
      const psCallsAfterFirst = psSpy.mock.calls.filter(
        (c) => c[0] === "ps",
      ).length;
      ...
      const psCallsAfterLoop = psSpy.mock.calls.filter(
        (c) => c[0] === "ps",
      ).length;
...
describe("killProcessTree — signal first, sweep second", () => {
  ...
  it("a real descendant tree is reaped by the sweep (kill then descendants)", async () => {
    const child = spawn("bash", ["-c", "sleep 30 & exec sleep 30"], {
      detached: true,
      stdio: "ignore",
    });
    child.unref();
    ...
    let waited = 0;
    while (waited < 1000) {
      const allDead = [pid, ...descendants].every((p) => {
        try {
          process.kill(p, 0);
          return false;
        } catch {
...
describe("runWorker — no ps on the watchdog hot path", () => {
  it('a healthy worker resolves "done" without any ps spawn from the watchdog', async () => {
    ...
    const spy = spyOn(childProcess, "spawnSync").mockImplementation(((
      ...callArgs: Parameters<typeof spawnSync>
    ) => {
      const [cmd, args] = callArgs;
      if (cmd === "ps" || cmd === "pgrep")
        psCalls.push([cmd, ...(args ?? [])].join(" "));
      if (cmd === "ps")
        return { status: 0, stdout: "00:01", stderr: "" } as ReturnType<
          typeof spawnSync
        >;
    ...
    try {
      const r = await runWorker(
        fakeScript("echo hi > snappy-output.txt; echo DONE"),
        REPO,
        handle,
        "cmd",
        lp,
